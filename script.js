...
// 2.
const Cars = data.filter(person => person.car_year > 2000);
const newcars = Cars.map(person => person.id);
console.log(newcars)



// 3. 
const bySize = data.filter(person => person.shirt_size === 'XS' || person.shirt_size === 'S');
const sortByName = bySize.sort((a, b) => a.first_name.localeCompare(b.first_name));
